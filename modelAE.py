...
class IM_AE(object):
	...
	# NEW! validation
	def validate(self):
		shape_num = len(self.val_data_voxels)
		...
		for idx in range(batch_num):
			...
			batch_voxels = self.val_data_voxels[dxb].astype(np.float32)
			if point_batch_num==1:
				point_coord = self.val_data_points[dxb]
				point_value = self.val_data_values[dxb]
			else:
				which_batch = np.random.randint(point_batch_num)
				point_coord = self.val_data_points[dxb,which_batch*self.point_batch_size:(which_batch+1)*self.point_batch_size]
				point_value = self.val_data_values[dxb,which_batch*self.point_batch_size:(which_batch+1)*self.point_batch_size]
	...
	def train(self, config):
		...
		for epoch in range(0, training_epoch):
			...
			if epoch%20==19:
				...
				score = self.validate()
				score_save_dir = os.path.join(self.checkpoint_path,self.checkpoint_name+str(self.sample_vox_size)+"-"+str(epoch)+".json")
				if not os.path.exists(os.path.dirname(score_save_dir)):
					os.makedirs(os.path.dirname(score_save_dir))
				with open(score_save_dir, 'w') as f:
					json.dump({'average_batch_sp': score}, f)

				## NEW! we want to keep the different checkpoints to select the one with the least validation loss.
				## delete checkpoint
	...
	def get_z(self, config):
		...
		hdf5_path = self.checkpoint_dir+'/'+self.model_dir+'/'+self.dataset_name+config.z_postfix #'_train_z.hdf5'
		print(hdf5_path)
		shape_num = len(self.data_voxels)
